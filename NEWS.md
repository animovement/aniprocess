...
* New filter `filter_ccma()`: Curvature-Corrected Moving Average
  ...
  spherical aniframes via `aniframe::ensure_is_cartesian()` because
  the underlying curvature math (cross product, Euclidean norm,
  circumradius) is Cartesian-specific. Hanning and uniform kernels
  supported in this release; padding boundary mode (#11).
